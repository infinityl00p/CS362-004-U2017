/**************************************
    Name: James Gill
    Date: July.30/2017
    Description: Random test of village card, should give you +1 card
...
int checkVillageCard(int p, struct gameState *post, int handCount) {
    ...
    //add 1 card from deck
    for(numCards = 0; numCards < 1; numCards++) {
        drawCard(p, &pre);
    }

    r = villageAction(p, post, handCount);
        ...
        for (deckCount = 0; deckCount < 5; deckCount++) {
            ...
            for (discardCount = 0; discardCount < 5; discardCount++) {
                ...
                for (handCount = 0; handCount < 5; handCount++) {
                    ...
                    checkVillageCard(p, &state, handCount);
                }
            }
        }
    }
